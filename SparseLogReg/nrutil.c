...
float *vector(int nl,int nh)
{
	...
	//v=(float *)R_alloc((nh-nl+1), sizeof(float));
...
unsigned long *lvector(int nl, int nh)
{
    ...
    //v=(unsigned long *)R_alloc((nh-nl+1), sizeof(long)); 
...
int *ivector(int nl,int nh)
{
	...
	//v=(int *)R_alloc((nh-nl+1), sizeof(int));
...
unsigned char *cvector(int nl,int nh)
{
	...
	//v=(unsigned char *)R_alloc((nh-nl+1), sizeof(unsigned char));
	if (!v) nrerror("allocation failure in ivector()");
	return v-nl;
}
...
double *dvector(int nl,int nh)
{
	...
	//v=(double *)R_alloc((nh-nl+1), sizeof(double));
...
float **matrix(int nrl,int nrh,int ncl,int nch)
{
	...
	//m=(float **) R_alloc((nrh-nrl+1), sizeof(float*));
	if (!m) nrerror("allocation failure 1 in matrix()");
	m -= nrl;

	for(i=nrl;i<=nrh;i++) {
		m[i]=(float *) malloc((unsigned) (nch-ncl+1)*sizeof(float));
		//m[i]=(float *) R_alloc((nch-ncl+1), sizeof(float));
...
double **dmatrix(int nrl,int nrh,int ncl,int nch)
{
	...
	//m=(double **) R_alloc((nrh-nrl+1), sizeof(double*));
	if (!m) nrerror("allocation failure 1 in dmatrix()");
	m -= nrl;

	for(i=nrl;i<=nrh;i++) {
		m[i]=(double *) malloc((unsigned) (nch-ncl+1)*sizeof(double));
		//m[i]=(double *) R_alloc((nch-ncl+1), sizeof(double));
...
int **imatrix(int nrl,int nrh,int ncl,int nch)
{
  ...
  //m=(int **)R_alloc((nrh-nrl+1), sizeof(int*));
  if (!m) nrerror("allocation failure 1 in imatrix()");
  m -= nrl;
  
  for(i=nrl;i<=nrh;i++) {
    m[i]=(int *)malloc((unsigned) (nch-ncl+1)*sizeof(int));
    //m[i]=(int *)R_alloc((nch-ncl+1), sizeof(int));
...
unsigned char **cmatrix(int nrl,int nrh,int ncl,int nch)
{
  ...
  //m=(unsigned char **)R_alloc((nrh-nrl+1), sizeof(unsigned char*));
  if (!m) nrerror("allocation failure 1 in cmatrix()");
  m -= nrl;
  
  for(i=nrl;i<=nrh;i++) {
    m[i]=(unsigned char *)malloc((unsigned) (nch-ncl+1)*sizeof(unsigned char));
    //m[i]=(unsigned char *)R_alloc((nch-ncl+1), sizeof(unsigned char));
		  ...
		  int newrl,int newcl)
{
	...
	//m=(float **) R_alloc((oldrh-oldrl+1), sizeof(float*));
...
float **convert_matrix(float *a,int nrl,int nrh,int ncl,int nch)
{
	...
	//m = (float **) R_alloc((nrow), sizeof(float*));
